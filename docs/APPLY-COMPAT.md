...
Last updated: PLAN-08 implementation (2026-08-23). Runtime evidence is
strongest on Windows; macOS/Linux rows describe implemented paths whose
real-app coverage is still limited. Absence from a table means "untested".
...
**Apply caveat (honest scope):** the _end-to-end synthetic paste_ into
...
## Native apps (macOS and Linux)

| Platform                    | Detection                              | Widget      | Apply       | Notes                                                                              |
| --------------------------- | -------------------------------------- | ----------- | ----------- | ---------------------------------------------------------------------------------- |
| macOS Accessibility clients | Implemented, runtime coverage pending  | Implemented | Unsupported | Requires Accessibility permission; AX offset/apply probe results are still pending |
| Linux X11 AT-SPI2 clients   | Implemented, runtime coverage pending  | Implemented | Unsupported | Requires `at-spi2-core`; AT-SPI2 field reading is fail-closed                      |
| Linux Wayland               | Implemented where AT-SPI2 is available | Implemented | Unsupported | Wayland does not permit the global synthetic input needed by native apply          |

Native fix application remains Windows-only. Text-expansion snippets are also
Windows-only. These are explicit unsupported outcomes, not silent no-ops.
