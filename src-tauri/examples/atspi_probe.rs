...
async fn find_focused_text(
    ...
    let mut queue: VecDeque<(Option<UniqueName<'static>>, ObjectPath<'static>)> =
        root.get_children().await.ok()?.into_iter()
            .map(|r| (r.name().cloned(), r.path().clone()))
            .collect();

    while let Some((name, path)) = queue.pop_front() {
        let Some(name) = name else { continue };
        let Some(node) = accessible(conn, &name, &path).await else { continue };
...
pub(super) fn run() {
    ...
    print!(
        "Type/paste the marker into gedit's focused document, then press Enter here... "
    );
